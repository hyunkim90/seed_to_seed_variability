...
physeq_B<-bac.clean.rel
sd<-sample_data(bac.clean.rel)
sd <- data.frame(sd)
tax<-tax_table(bac.clean.rel)
...
write.csv(Bnull.out, "bnullout_bacteria_seed_relative abundance.csv") #Branch

# upload BC null deviations of bacteria
Bnull=read.csv("bnullout_bacteria_seed_relative abundance.csv", row.names=1, header=T)
...
physeq_F<-fun.clean.rel
sd<-sample_data(fun.clean.rel)
sd <- data.frame(sd)
tax<-tax_table(fun.clean.rel)
...
write.csv(Bnull.out, "bnullout_fungi_seed_relative abundance.csv")

# upload BC null deviations of bacteria
Bnull=read.csv("bnullout_fungi_seed_relative abundance.csv", row.names=1, header=T)
DT.B=data.table(Bnull, keep.rownames=T, key="rn")
setnames(DT.B, c("BRAY_beta_div_abund_stoch", "BRAY_AbundanceNullDeviation"), c("BCF","NullB"))
sd <- data.frame(sd)
DT.sd=data.table(sd, keep.rownames=T, key="rn")
...
physeq_SUB <- bac.clean.rel


### Fungi
Bnull<-list()
Bnull.out<-data.frame()
physeq_B<-fun.clean.rel
sd<-sample_data(fun.clean.rel)
sd <- data.frame(sd)
tax<-tax_table(fun.clean.rel)

physeq_SUB <- fun.clean.rel
...
write.csv(Bnull.out, "Bull_bacteria_all seeds_rel.csv")
write.csv(Bnull.out, "Bull_fungi_all seeds_rel.csv")

Bnull=read.csv("Bull_bacteria_all seeds_rel.csv", row.names=1, header=T)
DT.B=data.table(Bnull, keep.rownames=T, key="rn")
setnames(DT.B, c("BRAY_beta_div_abund_stoch", "BRAY_AbundanceNullDeviation"), c("BCF","NullB"))
sd <- data.frame(sd)
DT.sd=data.table(sd, keep.rownames=T, key="rn")
...
Bnull=read.csv("Bull_fungi_all seeds_rel.csv", row.names=1, header=T)
DT.B=data.table(Bnull, keep.rownames=T, key="rn")
setnames(DT.B, c("BRAY_beta_div_abund_stoch", "BRAY_AbundanceNullDeviation"), c("BCF","NullB"))
sd <- data.frame(sd)
DT.sd=data.table(sd, keep.rownames=T, key="rn")
...
write.csv(DT.m,"Raw table for plotting beta null deviation_relative abundance.csv")
